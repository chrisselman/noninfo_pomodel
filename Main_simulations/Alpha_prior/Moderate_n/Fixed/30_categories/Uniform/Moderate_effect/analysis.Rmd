...
for(i in 1:nsim){
  MC_YMinusi_relbias_dirone[i] = (exp(sum(bias_dirones[-i])/(nsim-1)+ log(1.50))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias_dirzer[i] = (exp(sum(bias_dirzero[-i])/(nsim-1)+ log(1.50))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias_dirje[i] = (exp(sum(bias_dirjef[-i])/(nsim-1)+ log(1.50))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias_dirre[i] = (exp(sum(bias_dirrec[-i])/(nsim-1)+ log(1.50))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias_norm[i] = (exp(sum(bias_normal[-i])/(nsim-1)+ log(1.50))-exp(log(1.5)))/(exp(log(1.5)))
}
...
write.csv(results,file="G:/BACKUP/Chris/Project_3/noninfo_pomodel/Main_Simulations/Results/results_199.csv")
```
