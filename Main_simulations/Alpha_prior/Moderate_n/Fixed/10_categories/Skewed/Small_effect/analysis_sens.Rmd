...
```{r}
relbias_dirone <- (exp(bias_dirone+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_dirzer <- (exp(bias_dirzer+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_dirje <- (exp(bias_dirje+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_dirre <- (exp(bias_dirre+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_normal <- (exp(bias_norm+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
...
for(i in 1:(nsim-numdivones)){
  MC_YMinusi_relbias_dirone[i] = (exp(sum(bias_dirones[-i],na.rm=T)/(nsim-numdivones-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
}

for(i in 1:(nsim-numdivzero)){
  MC_YMinusi_relbias_dirzer[i] = (exp(sum(bias_dirzero[-i],na.rm=T)/(nsim-numdivzero-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
}

for(i in 1:(nsim-numdivjef)){
  MC_YMinusi_relbias_dirje[i] = (exp(sum(bias_dirjef[-i],na.rm=T)/(nsim-numdivjef-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
}

for(i in 1:(nsim-numdivrec)){
  MC_YMinusi_relbias_dirre[i] = (exp(sum(bias_dirrec[-i],na.rm=T)/(nsim-numdivrec-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
}

for(i in 1:(nsim-numdivnormal)){
  MC_YMinusi_relbias_norm[i] = (exp(sum(bias_normal[-i],na.rm=T)/(nsim-numdivnormal-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
}
...
write.csv(results,file="G:/BACKUP/Chris/Project_3/Main_Simulations/Results/results_88_sens.csv")
```
