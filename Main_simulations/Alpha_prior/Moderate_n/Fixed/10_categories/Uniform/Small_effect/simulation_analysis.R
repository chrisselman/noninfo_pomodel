...
setwd("/group/cebu1/BACKUP/Chris/Project_3/noninfo_pomodel/Main_simulations/Alpha_prior/Moderate_n/Fixed/10_categories/Uniform/Small_effect")
...
n_states = 10 ## Number of levels in ordinal outcome 
...
bias_dirones[k] <- as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1)
...
coverage_dirones[k] <- ifelse(lci <= log(1.1) & uci >= log(1.1), 1, 0)

# MSE 
mse_dirones[k] <- (as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1))^2
...
# Extract summary for proportional OR 
# Bias for each cumulative logit 
bias_dirzero[k] <- as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1)
...
coverage_dirzero[k] <- ifelse(lci <= log(1.1) & uci >= log(1.1), 1, 0)

# MSE 
mse_dirzero[k] <- (as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1))^2
...
# Extract summary for proportional OR 
# Bias for each cumulative logit 
bias_dirjef[k] <- as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1)
...
coverage_dirjef[k] <- ifelse(lci <= log(1.1) & uci >= log(1.1), 1, 0)

# MSE 
mse_dirjef[k] <- (as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1))^2
...
# Extract summary for proportional OR 
# Bias for each cumulative logit 
bias_dirrec[k] <- as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1)
...
coverage_dirrec[k] <- ifelse(lci <= log(1.1) & uci >= log(1.1), 1, 0)

# MSE 
mse_dirrec[k] <- (as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1))^2
...
# Extract summary for proportional OR 
# Bias for each cumulative logit 
bias_normal[k] <- as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1)
...
coverage_normal[k] <- ifelse(lci <= log(1.1) & uci >= log(1.1), 1, 0)

# MSE 
mse_normal[k] <- (as.numeric(summarise_draws(df, "median")[1,2]) - log(1.1))^2
...
bias_freq[k] <- as.numeric(freqmod$coefficients) - log(1.1)

coverage_freq[k] <- ifelse(as.numeric(confint(freqmod)[1]) <= log(1.1) & as.numeric(confint(freqmod)[2]) >= log(1.1), 1, 0)

mse_freq[k] <-  (as.numeric(freqmod$coefficients) - log(1.1))^2
}
...
data <- data.frame(
                   ...
                   tailess_dirones = tailess_dirones, postprob_dirones = postprob_dirones, numdivergent_dirones = numdivergent_dirones,
                   ...
                   tailess_dirzero = tailess_dirzero, postprob_dirzero = postprob_dirzero, numdivergent_dirzero = numdivergent_dirzero,
                   ...
                   tailess_dirjef = tailess_dirjef, postprob_dirjef = postprob_dirjef, numdivergent_dirjef = numdivergent_dirjef,
                   ...
                   tailess_dirrec = tailess_dirrec, postprob_dirrec = postprob_dirrec, numdivergent_dirrec = numdivergent_dirrec,
                   ...
                   tailess_normal = tailess_normal, postprob_normal = postprob_normal, numdivergent_normal = numdivergent_normal,
