...
```{r}
relbias_rsq <- (exp(bias_rsq)-exp(log(1.00)))/(exp(log(1.00)))
relbias_normlargesd <- (exp(bias_normlargesd)-exp(log(1.00)))/(exp(log(1.00)))
relbias_normsmallsd <- (exp(bias_normsmallsd)-exp(log(1.00)))/(exp(log(1.00)))
relbias_laplargesd <- (exp(bias_laplargesd)-exp(log(1.00)))/(exp(log(1.00)))
relbias_lapsmallsd <- (exp(bias_lapsmallsd)-exp(log(1.00)))/(exp(log(1.00)))
relbias_cauchy <- (exp(bias_cauch)-exp(log(1.00)))/(exp(log(1.00)))
relbias_frequentist <- (exp(bias_frequentist)-exp(log(1.00)))/(exp(log(1.00)))
```
...
write.csv(results,file="G:/BACKUP/Chris/Project_3/Main_Simulations/Results/results_5.csv")
```
