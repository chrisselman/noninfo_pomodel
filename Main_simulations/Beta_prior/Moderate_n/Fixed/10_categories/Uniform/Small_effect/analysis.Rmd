...
```{r}
relbias_rsq <- (exp(bias_rsq + log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_normlargesd <- (exp(bias_normlargesd+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_normsmallsd <- (exp(bias_normsmallsd+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_laplargesd <- (exp(bias_laplargesd+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_lapsmallsd <- (exp(bias_lapsmallsd+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_cauchy <- (exp(bias_cauch+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
relbias_frequentist <- (exp(bias_frequentist+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
```
...
for(i in 1:nsim){
  MC_YMinusi_relbias_rsquare[i] = (exp(sum(bias_rsquare[-i])/(nsim-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
  MC_YMinusi_relbias_normlargesd[i] = (exp(sum(bias_normal_largesd[-i])/(nsim-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
  MC_YMinusi_relbias_normsmallsd[i] = (exp(sum(bias_normal_smallsd[-i])/(nsim-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
  MC_YMinusi_relbias_laplargesd[i] = (exp(sum(bias_laplace_largesd[-i])/(nsim-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
  MC_YMinusi_relbias_lapsmallsd[i] = (exp(sum(bias_laplace_smallsd[-i])/(nsim-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
  MC_YMinusi_relbias_cauch[i] = (exp(sum(bias_cauchy[-i])/(nsim-1)+ log(1.1))-exp(log(1.1)))/(exp(log(1.1)))
}
...
effectsize <- rep("Small [OR = 1.10]", 7)
...
write.csv(results,file="G:/BACKUP/Chris/Project_3/noninfo_pomodel/Main_Simulations/Results/results_165.csv")
```
