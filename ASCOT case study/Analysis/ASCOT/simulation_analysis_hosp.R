...
example_data <- list(
  y = as.integer(d$dayhospitalisation) + 1, ## Outcome 
  n_states = 29, # Number of categories
  n_patients = length(d$dayhospitalisation), ## Sample size 
...
## R-square 
example_data <- list(
  y = as.factor(d$dayhospitalisation), ## Outcome 
  x = x) ## Treatment variable
...
postprob_rsquare <- (dim(df[df$x > 0,])[1])/length(df$x) 
  ...
  save(data, file = "hosp_beta_data.Rdata")
  write.csv(data,file="hosp_beta_data.csv")
  library(ggplot2)
  ...
  ggplot(data, aes(x=Prior, y = est, color = Prior)) +
    ...
    scale_y_continuous(breaks = c(-1,-0.75,-0.5,-0.25,0,0.25), labels = c(-1,-0.75,-0.5,-0.25,0,0.25)) +
    theme(axis.text=element_text(size=12),
    axis.title=element_text(size=14,face="bold"))
  ggsave("casestudy_hosp_beta.png",width = 12, height = 12)
...
## Do the exact same but changing the alpha priors 
  ...
  data <- data.frame(est = est, lci = lci, uci = uci, Prior = Prior, postprob = postprob)
  save(data, file = "hosp_alpha_data.Rdata")
  write.csv(data,file="hosp_alpha_data.csv")
  library(ggplot2)
  pd <- position_dodge(0.68)
  ...
  ggplot(data, aes(x=Prior, y = est, color = Prior)) +
    ...
    theme(axis.text=element_text(size=12),
          axis.title=element_text(size=14,face="bold"))
  ggsave("casestudy_hosp_alpha.png",width = 12, height = 12)
