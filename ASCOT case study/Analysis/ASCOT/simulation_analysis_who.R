...
example_data <- list(
  y = as.integer(d$who), ## Outcome 
  n_states = 8, # Number of categories
  n_patients = length(d$who), ## Sample size 
  x = x, ## Treatment variable
  p_par = rep(1, 8), ## Concentration parameter for prior on alpha 
...
postprob_rsquare <- (dim(df[df$x < 0,])[1])/length(df$x) 
  ...
  save(data, file = "who_beta_data.Rdata")
  write.csv(data,file="/group/cebu1/BACKUP/Chris/Project_3/ASCOT/who_beta_data.csv")
  library(ggplot2)
  ...
  ggplot(data, aes(x=Prior, y = est, color = Prior)) +
    ...
    scale_y_continuous(breaks = c(-0.3,-0.2,-0.1,0,0.1,0.2), labels = c(-0.3,-0.2,-0.1,0,0.1,0.2)) +
    theme(axis.text=element_text(size=12),
    axis.title=element_text(size=14,face="bold"))
  ggsave("casestudy_who_beta.png",width = 12, height = 12)
...
## Do the exact same but changing the alpha priors 
  ...
  example_data <- list(
    ...
    n_patients = length(d$who), ## Sample size 
    x = x, ## Treatment variable
    p_par = rep(0.5, 8), ## Concentration parameter for prior on alpha 
    sdbeta = sdbeta ## Specify prior SD for log-OR 
  )
  ...
  example_data <- list(
    ...
    n_patients = length(d$who), ## Sample size 
    x = x, ## Treatment variable
    p_par = rep(1/8, 8), ## Concentration parameter for prior on alpha 
    sdbeta = sdbeta ## Specify prior SD for log-OR 
  )
  ...
  example_data <- list(
    ...
    n_patients = length(d$who), ## Sample size 
    x = x, ## Treatment variable
    p_par = rep(0.001, 8), ## Concentration parameter for prior on alpha 
    sdbeta = sdbeta ## Specify prior SD for log-OR 
  )
  ...
  data <- data.frame(est = est, lci = lci, uci = uci, Prior = Prior, postprob = postprob)
  save(data, file = "who_alpha_data.Rdata")
  write.csv(data,file="/group/cebu1/BACKUP/Chris/Project_3/ASCOT/who_alpha_data.csv")
  ...
  ggplot(data, aes(x=Prior, y = est, color = Prior)) +
    ...
    theme(axis.text=element_text(size=12),
          axis.title=element_text(size=14,face="bold"))
  ggsave("casestudy_who_alpha.png",width = 12, height = 12)
